...
var Demo = function() {
	var runDemoForms = function() {
		/*
		 * $('.admin-form').on( 'submit', function(e) { if
		 * ($('body.timeline-page').length ||
		 * $('body.admin-validation-page').length) { return; } e.preventDefault;
		 * alert('Your form has submitted!'); return false; });
		 */
		var fileUpload = $('.fileupload-preview');
		if (fileUpload.length) {
			fileUpload.each(function(i, e) {
				var fileForm = $(e).parents('.fileupload').find(
						'.btn-file > input');
				$(e).on('click', function() {
					fileForm.click();
				});
		...
		if (bsElement.length) {
			...
			codeBtn.click(function() {
				...
				$('.btn-clipboard').on(
						'click',
						function() {
							var selection = $(this).parents('.modal-dialog')
									.find('pre');
